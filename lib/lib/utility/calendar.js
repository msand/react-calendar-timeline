...
function stack(items, groupOrders, lineHeight, groups) {
  var groupMargin = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 0;
  ...
  groupedItems.forEach(function (group) {
    ...
    var height = groupVal.height || Math.max(groupHeight + verticalMargin, lineHeight);
...
function nostack(items, groupOrders, lineHeight, groups) {
  ...
  groupedItems.forEach(function (group) {
    ...
    var height = groupVal.height || Math.max(groupHeight, lineHeight);
    groupHeights.push(height);
    totalHeight += groupMargin + height;
  });
  return {
...
function stackItems(items, groups, canvasTimeStart, visibleTimeStart, visibleTimeEnd, width, props, state) {
  ...
  var _stackingMethod = stackingMethod(dimensionItems, groupOrders, lineHeight, groups, groupMargin),
