...
function (_Component) {
  ...
  function ReactCalendarTimeline(_props) {
    ...
    _defineProperty(_assertThisInitialized(_this), "changeZoom", function (scale) {
      ...
      var oldZoom = visibleTimeEnd - visibleTimeStart;
      var newScale = Math.min(Math.max(minScale, scale), maxScale);
      var newZoom = Math.min(Math.max(Math.round(oldZoom * newScale), minZoom), maxZoom); // min 1 min, max 20 years

      var sidebarWidth = _this.state.sidebarWidth || _this.props.sidebarWidth;

      if (sidebarWidth) {
        offset -= sidebarWidth / width;
      }

      var newVisibleTimeStart = Math.round(visibleTimeStart + (oldZoom - newZoom) * offset);
      onTimeChange(newVisibleTimeStart, newVisibleTimeStart + newZoom, _this.updateScrollCanvas);
    });
    ...
    _defineProperty(_assertThisInitialized(_this), "onResizeSidebar", function (isRightSidebar, width) {
      _this.setState(isRightSidebar ? {
  ...
  _createClass(ReactCalendarTimeline, [{
    ...
    key: "items",
    value: function items(canvasTimeStart, zoom, canvasTimeEnd, canvasWidth, minUnit, dimensionItems, groupHeights, groupTops, headerHeight) {
    ...
    value: function sidebar(height, groupHeights, headerHeight) {
      var sidebarWidth = this.state.sidebarWidth || this.props.sidebarWidth;
    ...
    value: function rightSidebar(height, groupHeights, headerHeight) {
      var rightSidebarWidth = this.state.rightSidebarWidth || this.props.rightSidebarWidth;
    ...
    value: function render() {
      ...
      var _this$props2 = this.props,
          ...
          _this$props2$headerHe = _this$props2.headerHeight,
          headerHeight = _this$props2$headerHe === void 0 ? 60 : _this$props2$headerHe;
      ...
      return _react["default"].createElement(_TimelineStateContext.TimelineStateProvider, {
        ...
        className: "rct-outer"
      }, _react["default"].createElement(_ScrollElement["default"], {
      ...
      }, this.renderHeaders(), sidebarWidth > 0 ? this.sidebar(height, groupHeights, headerHeight) : null, _react["default"].createElement("div", {
        style: {
          position: 'relative',
          display: 'inline'
        }
      }, _react["default"].createElement(_MarkerCanvas["default"], null, this.columns(canvasTimeStart, canvasTimeEnd, canvasWidth, minUnit, timeSteps, height), this.rows(canvasWidth, groupHeights, groups), this.items(canvasTimeStart, zoom, canvasTimeEnd, canvasWidth, minUnit, dimensionItems, groupHeights, groupTops, headerHeight), this.childrenWithProps(canvasTimeStart, canvasTimeEnd, canvasWidth, dimensionItems, groupHeights, groupTops, height, visibleTimeStart, visibleTimeEnd, minUnit, timeSteps)))), rightSidebarWidth > 0 ? this.rightSidebar(height, groupHeights, headerHeight) : null)))));
    }
  }], [{
